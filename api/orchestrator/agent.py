...
def create_research_agent(vector_store_id: Optional[str] = None) -> Agent:
    ...
    async def call_runtime_tool(
        ...
        if tool_name == "search_web_sources":
            ...
            if normalized_query in {" ".join(existing.lower().split()) for existing in state.search_queries}:
                return {
                    ...
                    "budget": {
                        ...
                        "search_calls_remaining": max(0, MAX_SEARCH_CALLS - state.search_call_count),
                    },
                }
            ...
            if state.search_call_count >= MAX_SEARCH_CALLS:
                return {
                    "tool_name": tool_name,
            ...
            try:
                payload = await call_runtime_tool_via_mcp_or_local(
                    tool_name,
                    {"query": query, "max_results": min(arguments.get("max_results", 5), 2)},
                )
        ...
        if tool_name == "fetch_page":
                ...
                return {
                    **cached_page.to_agent_payload(),
                    ...
                    "budget": {
                        ...
                        "fetch_calls_remaining": max(0, MAX_FETCH_CALLS - state.fetch_call_count),
                    },
                }
            ...
            if state.fetch_call_count >= MAX_FETCH_CALLS:
                return {
                    "url": url,
        ...
        if tool_name == "browse_page_tool":
            ...
            if state.fetch_call_count >= MAX_FETCH_CALLS or "concise" in concise_mission:
                return {
                    "url": url,
                    "fetch_status": "budget_exhausted",
        ...
        if tool_name == "retrieve_evidence_chunks":
            payload = await call_runtime_tool_via_mcp_or_local(
                tool_name,
                {"claim": arguments["claim"], "source_urls": arguments["source_urls"]},
            )
            chunks = [EvidenceChunk.model_validate(chunk) for chunk in payload.get("chunks", [])]
            for chunk in chunks:
                state.evidence_chunks[chunk.chunk_id] = chunk
        ...
        if tool_name == "verify_claim":
            ...
            result = ClaimSupportResult.model_validate(payload)
